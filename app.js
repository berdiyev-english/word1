// English Words App - Fixed navigation, separated sections, and words rendering

class EnglishWordsApp { constructor() { this.currentSection = 'about'; this.currentLevel = null; this.learningWords = []; this.customWords = []; this.audioPlayer = document.getElementById('audioPlayer'); this.currentAudioUrl = null;
...
// Data Management (storage only, UI for it removed)
loadData() {
    try {
...
// Events
setupEventListeners() {
    // Delegated bottom nav clicks to make it robust
    ...
    // Level cards (click to open words)
...
renderLevelWords(level) {
    ...
    const db = oxfordWordsDatabase[level] || [];
    ...
    wordsList.innerHTML = words.map(wordData => {
        ...
        const w = this.safe(wordData.word);
        const t = this.safe(wordData.translation);
        return `
            <div class="word-card">
...
addAllFromLevel(level) {
    ...
    pool.forEach(w => {
        if (!this.learningWords.find(x => x.word === w.word)) {
            const newWord = this.createLearningWord(w.word, w.translation, level);
...
addToLearning(word, translation, level) {
    ...
    const newWord = this.createLearningWord(word, translation, level);
    this.learningWords.push(newWord);
    this.saveData();
    this.updateUI();
...
showAllLearningWords() {
    ...
    filterButtons.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            if (filter === 'learning') filteredWords = learningWords;
            else if (filter === 'learned') filteredWords = learnedWords;
        ...
        return `
            <div class="word-card">
                <div class="word-header">
                    <span class="word-text">${wordData.word}</span>
                    <div class="word-actions">
                        <button class="action-btn play-btn" onclick="app.playAudio('${this.safe(wordData.word)}')" title="Прослушать произношение">
                            <i class="fas fa-play"></i>
                        </button>
                        ${!isLearning ? `
                            <button class="action-btn add-btn" onclick="app.addToLearning('${this.safe(wordData.word)}', '${this.safe(wordData.translation)}', '${wordData.level}')" title="Добавить в изучение">
                                <i class="fas fa-plus"></i>
                            </button>
                        ` : `
                            <button class="action-btn remove-btn" onclick="app.removeFromLearning('${this.safe(wordData.word)}')" title="Убрать из изучения">
                                <i class="fas fa-minus"></i>
                            </button>
                        `}
...
async playAudio(word, accent = 'uk') {
    ...
    const playButtons = document.querySelectorAll(`[onclick*="playAudio('${this.safe(word)}')"]`);
...
updateSpacedRepetition(word, quality) {
    ...
    if (quality >= 3) {
        ...
        data.repetitions++;
        data.difficulty = Math.max(0, data.difficulty - (quality === 5 ? 0.2 : 0.1));
    } else {
        data.repetitions = 0;
        data.interval = 1;
    ...
    nextReview.setDate(nextReview.getDate() + Math.max(1, data.interval));
    data.nextReview = nextReview.toISOString();
    data.lastReview = new Date().toISOString();

    if (data.totalAnswers >= 3) {
        const acc = data.correctAnswers / data.totalAnswers;
        if (acc >= 0.8) data.difficulty = 0;
        else if (acc >= 0.6) data.difficulty = 1;
        else data.difficulty = 2;
    }
    if (data.repetitions >= 6 && data.interval >= 30) {
        word.isLearned = true;
...
migrateExistingWords() {
    ...
    this.learningWords.forEach(word => {
        if (!word.repetitionData) {
            word.repetitionData = this.defaultRepetitionData();
            migrated = true;
        }
    });
...
renderProgress() {
    ...
    const levelCards = levels.map(lvl => {
        const total = (oxfordWordsDatabase[lvl] || []).length + this.customWords.filter(w => w.level === lvl).length;
...
// Initialize app document.addEventListener('DOMContentLoaded', () => { window.app = new EnglishWordsApp(); });

// Optional styles for learned words const additionalStyles =
.word-card.learned { opacity: 0.7; background: var(--bg-tertiary); } .learned-badge { background: var(--accent-color); color: white; padding: 0.25rem 0.5rem; border-radius: var(--radius-sm); font-size: 0.75rem; margin-top: 0.5rem; display: inline-block; }
; const styleSheet = document.createElement('style'); styleSheet.textContent = additionalStyles; document.head.appendChild(styleSheet);
