class EnglishWordsApp {
    constructor() {
        ...
        this.directionMode = 'auto';     // случайное направление EN<->RU
    ...
    init() {
        ...
        if (!this.dbAvailable) {
            console.warn('oxford_words_data.js не найден или пуст. Слова по уровням будут пустыми.');
    ...
    /* ================= СОБЫТИЯ ================= */
    setupEventListeners() {
        // 1) Нижняя навигация — ДЕЛЕГИРОВАНИЕ (надежно)
        ...
        // 2) Нижняя навигация — ПРЯМЫЕ обработчики (резерв)
        ...
        // Переключатели режимов обучения
        ...
        // Массовые действия по уровню (прямые и делегирование)
    ...
    buildQuizOptions(currentWord, direction) {
        ...
        // Разрешаем 2-4 опции (если слов мало)
